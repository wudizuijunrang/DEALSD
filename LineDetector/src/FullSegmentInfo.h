#ifndef DEALSD_FULLSEGMENTINFO_H_
#define DEALSD_FULLSEGMENTINFO_H_
...
class FullSegmentInfo {
 public:
  ...
  FullSegmentInfo(const std::vector<Pixel> &pts, int width,int height,float* heatmap);
  // Creates the FullSegmentInfo object and initializes internal structures
  FullSegmentInfo(const std::vector<Pixel> &pts, int startIdx, int width, int height, float* heatmap);
  ...
  //Returns the confidence of the line segment
  inline float getAveHeat() const { return average_heat; }
 ...
 private:
  ...
  //computes the confidenc of the line segment
  void calculateHeat(const std::vector<Pixel>& pts, int startIndex = 0);
  ...
  float average_heat = 0;
  const float* pHeatmap;
  int imageWidth = 0;
  int imageHeight = 0;
};
