#ifndef DEALSD_EDGEDRAWER_H_
#define DEALSD_EDGEDRAWER_H_
...
class EdgeDrawer {
 ...
 private:
   ...
   * @return
   */
  inline static bool findNextPxWithProjection(const float *gradImg,
                                              cv::Vec3f eq,
   ...
   */
  void drawEdgeTreeStack(Pixel anchor, ImageEdge &initialPixels, bool firstAnchorDirection, int imageWidth, int imageHeright, float* heatmap);
  ...
  int minLineLength = 15;
  bool treatJunctions = true;
  std::vector<int> listOfJunctionSizes = {3, 5, 7};
  double junctionEigenvalsTh = 10;
  double junctionAngleTh = 10 * (M_PI / 180.0);
};
