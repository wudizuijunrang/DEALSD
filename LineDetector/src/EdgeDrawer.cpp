...
void EdgeDrawer::init(const LineDetectionExtraInfoPtr &gradientInfo, cv::Mat &edgeImage) {
  ...
  assert(gradientInfo->gImg.type() == CV_32FC1);
  gradImg = gradientInfo->gImg.ptr<float>();
  ...
  assert(gradientInfo->dxImg.type() == CV_32FC1);
  assert(gradientInfo->dxImg.size() == gradientInfo->gImg.size());
  pDxImg = gradientInfo->dxImg.ptr<float>();
  assert(!gradientInfo->dyImg.empty());
  assert(gradientInfo->dyImg.type() == CV_32FC1);
  assert(gradientInfo->dyImg.size() == gradientInfo->gImg.size());
  pDyImg = gradientInfo->dyImg.ptr<float>();
  assert(!gradientInfo->heatmap.empty());
  assert(gradientInfo->heatmap.type() == CV_32FC1);
  assert(gradientInfo->heatmap.size() == gradientInfo->gImg.size());
  pHeatmap = gradientInfo->heatmap.ptr<float>();
...
void EdgeDrawer::drawEdgeTreeStack(Pixel anchor, ImageEdge &initialPixels, bool firstAnchorDirection, int imageWidth, int imageHeight, float* heatmap) {
  ...
  segments.emplace_back(pixels,imageWidth,imageHeight,pHeatmap);
  localSegment = &segments.back(); //新的直线段
...
//         << "\n\t- Initial Pixels: " << branch.pixels;
    ...
    if (firstBranch && edgeImg[anchor.y * imageWidth + anchor.x] != UPM_ED_SEGMENT_INLIER_PX) {
      ...
      for (i = initialPxIndex + nElements - 1; i >= initialPxIndex; i--) { //将未被拟合成直线的像素点放入另一个枝干，作为后续延申
...
void EdgeDrawer::drawEdgeInBothDirections(uint8_t direction, Pixel anchor) {
  ...
  drawEdgeTreeStack(anchor, anchorPixels, true,imageWidth,imageHeight,pHeatmap);
  ...
  drawEdgeTreeStack(anchor, anchorPixels, false, imageWidth, imageHeight, pHeatmap);
}
...
bool EdgeDrawer::findNextPxWithProjection(const float *gradImg,
                                          cv::Vec3f eq,
  ...
  float gValue1, gValue2, gValue3, gValue4;
