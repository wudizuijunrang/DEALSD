...
FullSegmentInfo::FullSegmentInfo(const std::vector<Pixel> &pts, int width, int height, float* heatmap) : pixels(&pts) {
    pHeatmap = heatmap;
    imageWidth = width;
    imageHeight = height;
}

FullSegmentInfo::FullSegmentInfo(const std::vector<Pixel> &pts, int startIdx, int width, int height, float* heatmap) : pixels(&pts) {
  init(pts, startIdx);
  pHeatmap = heatmap;
  imageWidth = width;
  imageHeight = height;
...
void FullSegmentInfo::init(const std::vector<Pixel> &pts, int startIdx) {
  ...
  calculateHeat(pts, startIdx); //average confidence 
...
inline void FullSegmentInfo::calculateHeat(const std::vector<Pixel>& pts, int startIdx) { 
    average_heat = 0;
    int index;
    for (int i = startIdx; i < pts.size(); i++) {
        index = pts[i].y * imageWidth + pts[i].x;
        average_heat += pHeatmap[index];
    }
    average_heat /= N;
}
...
inline void FullSegmentInfo::leastSquaresLineFitNewPoint(int x, int y) {
  ...
  average_heat *= N;
  average_heat += pHeatmap[y * imageWidth + x];
  N++;
  average_heat /= N;
  sum_x_i += indpCoord;
  sum_y_i += depCoord;
...
inline void FullSegmentInfo::subtractPointFromModel(const Pixel &p) {
  ...
  average_heat *= N;
  average_heat -= pHeatmap[p.y * imageWidth + p.x];
  N--;
  average_heat /= N;
